...
d <- d[, -c(which(names(d) %in% c("MEAN_ANOMALY","ARG_OF_PERICENTER","RA_OF_ASC_NODE")))];

# Converting categorical variables into factors ----
categorical_columns <- c(10,11);
...
predictions1 <- predict(gbm_model, test_data[, -c(11)], type = "prob")

# ROC and AUC Plot 

# Create ROC curve object for each class
roc_curve_small <- roc(ifelse(test_data$RCS_SIZE == "SMALL", 1, 0), predictions1[, "SMALL"])
roc_curve_medium <- roc(ifelse(test_data$RCS_SIZE == "MEDIUM", 1, 0), predictions1[, "MEDIUM"])
roc_curve_large <- roc(ifelse(test_data$RCS_SIZE == "LARGE", 1, 0), predictions1[, "LARGE"])

# Calculate AUC for each class
auc_large1 <- auc(roc_curve_large)
auc_medium1 <- auc(roc_curve_medium)
auc_small1 <- auc(roc_curve_small)

# Plot ROC curves
plot(roc_curve_small, main = "ROC Curves (GBM)", col = "red", lwd = 2)
plot(roc_curve_medium, col = "green", add = TRUE, lwd = 2)
plot(roc_curve_large, col = "blue", add = TRUE, lwd = 2)

# Add AUC to the plot
text(0.5, 0.3, paste("AUC Small =", round(auc(roc_curve_small), 3)), col = "red", cex = 1.2)
text(0.6, 0.2, paste("AUC Medium =", round(auc(roc_curve_medium), 3)), col = "green", cex = 1.2)
text(0.7, 0.1, paste("AUC Large =", round(auc(roc_curve_large), 3)), col = "blue", cex = 1.2)

# Add legend
legend("bottomright", legend = c("Small", "Medium", "Large"), col = c("red", "green", "blue"), lwd = 2)
...
print(paste("AUC Large : ",auc_large1));
print(paste("AUC Medium : ",auc_medium1));
print(paste("AUC SMALL : ",auc_small1));
...
# Make predictions using the trained SVM model on test data 
tp2 <- predict(rf_model, test_data[, -c(11)]); # column 11 is the target variable
...
# Create ROC curve object for each class
roc_curve_small <- roc(ifelse(test_data$RCS_SIZE == "SMALL", 1, 0), predictions2[, "SMALL"])
roc_curve_medium <- roc(ifelse(test_data$RCS_SIZE == "MEDIUM", 1, 0), predictions2[, "MEDIUM"])
roc_curve_large <- roc(ifelse(test_data$RCS_SIZE == "LARGE", 1, 0), predictions2[, "LARGE"])

# Calculate AUC for each class
auc_large2 <- auc(roc_curve_large)
auc_medium2 <- auc(roc_curve_medium)
auc_small2 <- auc(roc_curve_small)

# Plot ROC curves
plot(roc_curve_small, main = "ROC Curves (RF)", col = "red", lwd = 2)
plot(roc_curve_medium, col = "green", add = TRUE, lwd = 2)
plot(roc_curve_large, col = "blue", add = TRUE, lwd = 2)
...
print(paste("AUC Large : ",auc_large2));
print(paste("AUC Medium : ",auc_medium2));
print(paste("AUC SMALL : ",auc_small2));
...
# Make predictions using the trained SVM model on test data 
tp3 <- predict(mlr_model, test_data[, -c(11)]); # column 11 is the target variable
test_data$test_preds3 = tp3;
predictions3 <- predict(mlr_model, test_data[, -c(11)], type = "prob")

# ROC and AUC Plot 

# Create ROC curve object for each class
roc_curve_small <- roc(ifelse(test_data$RCS_SIZE == "SMALL", 1, 0), predictions3[, "SMALL"])
roc_curve_medium <- roc(ifelse(test_data$RCS_SIZE == "MEDIUM", 1, 0), predictions3[, "MEDIUM"])
roc_curve_large <- roc(ifelse(test_data$RCS_SIZE == "LARGE", 1, 0), predictions3[, "LARGE"])

# Calculate AUC for each class
auc_large3 <- auc(roc_curve_large)
auc_medium3 <- auc(roc_curve_medium)
auc_small3 <- auc(roc_curve_small)

# Plot ROC curves
plot(roc_curve_small, main = "ROC Curves (MLR)", col = "red", lwd = 2)
plot(roc_curve_medium, col = "green", add = TRUE, lwd = 2)
plot(roc_curve_large, col = "blue", add = TRUE, lwd = 2)
...
print(paste("AUC Large : ",auc_large3));
print(paste("AUC Medium : ",auc_medium3));
print(paste("AUC SMALL : ",auc_small3));
...
# Make predictions using the trained SVM model on test data 
tp4 <- predict(nb_model, test_data[, -c(11)]); # column 11 is the target variable
test_data$test_preds4 = tp4;
predictions4 <- predict(nb_model, test_data[, -c(11)], type = "raw")

# ROC and AUC Plot 

# Create ROC curve object for each class
roc_curve_small <- roc(ifelse(test_data$RCS_SIZE == "SMALL", 1, 0), predictions4[, "SMALL"])
roc_curve_medium <- roc(ifelse(test_data$RCS_SIZE == "MEDIUM", 1, 0), predictions4[, "MEDIUM"])
roc_curve_large <- roc(ifelse(test_data$RCS_SIZE == "LARGE", 1, 0), predictions4[, "LARGE"])

# Calculate AUC for each class
auc_large4 <- auc(roc_curve_large)
auc_medium4 <- auc(roc_curve_medium)
auc_small4 <- auc(roc_curve_small)

# Plot ROC curves
plot(roc_curve_small, main = "ROC Curves (NB)", col = "red", lwd = 2)
plot(roc_curve_medium, col = "green", add = TRUE, lwd = 2)
plot(roc_curve_large, col = "blue", add = TRUE, lwd = 2)
...
print(paste("AUC Large : ",auc_large4));
print(paste("AUC Medium : ",auc_medium4));
print(paste("AUC SMALL : ",auc_small4));

# Train the Support Vector Machine Model ----
cat("\nSupport Vector Machine (SVM) \n");

set.seed(123);
svm_model <- svm(formula = RCS_SIZE~., data = train_data, kernel = "linear",probability = TRUE);


# Make predictions using the trained SVM model on test data 
tp5 <- predict(svm_model, test_data[, -c(11)]); # column 12 is the target variable
test_data$test_preds5 = tp5;

predictions5 <- predict(svm_model, test_data[, -c(11)], probability = TRUE)

# Extract class probabilities for each class
probabilities <- attr(predictions5, "probabilities")

# ROC and AUC Plot 

# Create ROC curve object for each class
roc_curve_small <- roc(ifelse(test_data$RCS_SIZE == "SMALL", 1, 0), probabilities[, "SMALL"])
roc_curve_medium <- roc(ifelse(test_data$RCS_SIZE == "MEDIUM", 1, 0), probabilities[, "MEDIUM"])
roc_curve_large <- roc(ifelse(test_data$RCS_SIZE == "LARGE", 1, 0), probabilities[, "LARGE"])

# Calculate AUC for each class
auc_large5 <- auc(roc_curve_large)
auc_medium5 <- auc(roc_curve_medium)
auc_small5 <- auc(roc_curve_small)

# Plot ROC curves
plot(roc_curve_small, main = "ROC Curves (SVM)", col = "red", lwd = 2)
plot(roc_curve_medium, col = "green", add = TRUE, lwd = 2)
plot(roc_curve_large, col = "blue", add = TRUE, lwd = 2)
...
print(paste("AUC Large : ",auc_large5));
print(paste("AUC Medium : ",auc_medium5));
print(paste("AUC SMALL : ",auc_small5));
...
# Make predictions using the trained SVM model on test data 
tp6 <- predict(dt_model, test_data[, -c(11)]); # column 12 is the target variable
test_data$test_preds6 = tp6;

predictions6 <- predict(dt_model, test_data[, -c(11)], type = "prob")

# ROC and AUC Plot 

# Create ROC curve object for each class
roc_curve_small <- roc(ifelse(test_data$RCS_SIZE == "SMALL", 1, 0), predictions6[, "SMALL"])
roc_curve_medium <- roc(ifelse(test_data$RCS_SIZE == "MEDIUM", 1, 0), predictions6[, "MEDIUM"])
roc_curve_large <- roc(ifelse(test_data$RCS_SIZE == "LARGE", 1, 0), predictions6[, "LARGE"])

# Calculate AUC for each class
auc_large6 <- auc(roc_curve_large)
auc_medium6 <- auc(roc_curve_medium)
auc_small6 <- auc(roc_curve_small)

# Plot ROC curves
plot(roc_curve_small, main = "ROC Curves (DT)", col = "red", lwd = 2)
plot(roc_curve_medium, col = "green", add = TRUE, lwd = 2)
plot(roc_curve_large, col = "blue", add = TRUE, lwd = 2)
...
print(paste("AUC Large : ",auc_large6));
print(paste("AUC Medium : ",auc_medium6));
print(paste("AUC SMALL : ",auc_small6));
...
test_preds <- test_data[,c(12,13,14,15,16,17)];
